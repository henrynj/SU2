...
void CSurfaceMovement::SetSurface_Deformation(CGeometry *geometry, CConfig *config) {
  ...
  /*--- External surface file based ---*/
    ...
    if (Surface_File.fail()) {
      ...
      Surface_File.close();
      if (rank == MASTER_NODE) cout << "Reading the surface displacements from the input file." << endl;
  ...
  /*--- External surface file based ---*/
  else if (config->GetDesign_Variable(0) == SURFACE_DISPLACEMENT) {
    
    /*--- Check whether a surface file exists for input ---*/
    ofstream Displacement_File;
    string filename = config->GetMotion_FileName();
    Displacement_File.open(filename.c_str(), ios::in);
    
    /*--- A surface file does not exist, so write a new one for the
     markers that are specified as part of the motion. ---*/
    if (Displacement_File.fail()) {
      
      if (rank == MASTER_NODE)
        cout << "No surface file found. Writing a new file: " << filename << "." << endl;
      
      Displacement_File.open(filename.c_str(), ios::out);
      Displacement_File.precision(15);
      unsigned long iMarker, jPoint, GlobalIndex, iVertex; su2double *Coords;
      for (iMarker = 0; iMarker < config->GetnMarker_All(); iMarker++) {
        if (config->GetMarker_All_DV(iMarker) == YES) {
          for (iVertex = 0; iVertex < geometry->nVertex[iMarker]; iVertex++) {
            ...
            GlobalIndex = geometry->node[jPoint]->GetGlobalIndex();
            Coords = geometry->node[jPoint]->GetCoord();
            Displacement_File << GlobalIndex << "\t" << Coords[0] << "\t" << Coords[1];
            if (geometry->GetnDim() == 2) Displacement_File << endl;
            else Displacement_File << "\t" << Coords[2] << endl;
          }
        }
      }
      ...
      Displacement_File.close();
      if (rank == MASTER_NODE) cout << "Updating the surface coordinates from the input file generated by py." << endl;
      SetExternal_Displacement(geometry, config, ZONE_0, 0);
    }
...
void CSurfaceMovement::SetExternal_Displacement(CGeometry *geometry, CConfig *config, unsigned short iZone, unsigned long iter) {
  ...
  /*--- Local variables ---*/
	...
	unsigned long iPoint = 0, flowIter = 0;
  unsigned long jPoint, GlobalIndex;
	su2double *Coord_Old = NULL, *Coord_New = NULL;
  su2double Lref   = config->GetLength_Ref();
  su2double NewCoord[3] = {0.0,0.0,0.0};
  su2double r[3] = {0.0,0.0,0.0};
  unsigned long iVertex;
  unsigned short iMarker;
  char buffer[50];
  string motion_filename, text_line;
  ifstream motion_file;
  ...
  /*--- Throw error if there is no file ---*/
  if (motion_file.fail()) {
    cout << "There is no blade deformation file!" << endl;
    exit(EXIT_FAILURE);
  }
  
  /*--- Read in and store displacements on blade surface ---*/ 
  while (getline(motion_file, text_line)) {
    istringstream point_line(text_line);
    if (nDim == 2) point_line >> iPoint >> NewCoord[0] >> NewCoord[1];
    if (nDim == 3) point_line >> iPoint >> NewCoord[0] >> NewCoord[1] >> NewCoord[2];
